...
if [[ $1 = "create" ]]; then
  docker network create --subnet=172.20.0.0/16 my_subnet # create custom network

  # Starting Postresql Hive metastore
  echo ">> Starting postgresql hive metastore ..."
  docker run -d --net my_subnet --ip 172.20.1.2 -p 5432:5432 --hostname postgresqlnode --name postgresqlnode -e POSTGRES_PASSWORD=hive -it cluster:postgresql
  sleep 5
  
  # 3 nodes
  echo ">> Starting master node ..."
  docker run -d --net my_subnet --ip 172.20.1.1 -p 50070:50070 -p 50010:50010 -p 8088:8088 -p 8032:8032 -p 10000:10000 -p 7077:7077 -p 9000:9000 -p 19888:19888 -p 8080:8080 -p 18080:18080 --hostname masternode --add-host postgresqlnode:172.20.1.2 --add-host zookeepernode:172.20.1.3 --add-host kafkanode:172.20.1.4 --name masternode -it cluster:hive
